class ReceitaStorage{
	...
	salvaReceitas(listaReceitas){
	    ...
	    localStorage.setItem("receita", JSON.stringify(
	        listaReceitas.receitas.map(receita => ({data: receita.data, descricao: receita.descricao, tipo: receita.tipo, valor: receita.valor}))
	    ));
 	}
 	removeReceitas(){
